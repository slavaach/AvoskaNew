...
import ru.yandex.slavaach.nullapplicatoin.features.reference.AddDialog

class CustomFrgm : BaseFrgm(), Screen {
    ...
    @Composable
    fun GetView(
        viewModel: CustomViewModel = koinViewModel(),
    ) {
        val state = remember { viewModel.state }
        val stateList = viewModel.stateOld.collectAsState()
        val allList = stateList.value.list
        LaunchedEffect(0) {
            viewModel.setSubTitleName("Надо купить")
            viewModel.setIconTitle(R.drawable.shoping_set)
            viewModel.setTitleName()
            viewModel.setClickOnTitle(ClickOnTitle.CLICK_ONE)
        }
